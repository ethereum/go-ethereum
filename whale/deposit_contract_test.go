...
func TestReplaceDepositContract(t *testing.T) {
	...
	// Verify
	balance := state.GetBalance(whaleDepositContractAddr)
	if balance.Cmp(common.Big0) != 0 {
		t.Errorf("Found unexpected deposit contract balance: %d", balance)
	}

	actualCode := state.GetCode(whaleDepositContractAddr)
	...
	for i, store := range depositContractStorage {
		actualStorage := state.GetState(whaleDepositContractAddr, common.HexToHash(store[0]))
