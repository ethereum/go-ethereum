...
func testIndexWriterWithLimit(t *testing.T, bitmapSize int) {
	...
	for i := 0; i < 100; i++ {
		...
		var maxElem uint64
		for elem := limit + 1; elem < 4096*2; elem++ {
...
func testIndexDeleterBasic(t *testing.T, bitmapSize int) {
	...
	var maxElem uint64
	for i := 0; i < indexBlockSizeCap*4; i++ {
	...
	if err := id.pop(indexBlockSizeCap * 5); err == nil {
		t.Fatal("Expect error to occur for unknown id")
	}
	for i := indexBlockSizeCap * 4; i >= 1; i-- {
...
func testIndexDeleterWithLimit(t *testing.T, bitmapSize int) {
	...
	var maxElem uint64
	for i := 0; i < indexBlockSizeCap*2; i++ {
		iw.append(uint64(i+1), randomExt(bitmapSize, 5))
		maxElem = uint64(i + 1)
	}
	batch := db.NewBatch()
	...
	suites := []struct {
		...
		{
			uint64(indexBlockSizeCap + 1),
			uint64(indexBlockSizeCap + 1),
		},
		// truncation around the block boundary
		{
			uint64(indexBlockSizeCap),
			uint64(indexBlockSizeCap),
		},
		{
			uint64(1), uint64(1),
