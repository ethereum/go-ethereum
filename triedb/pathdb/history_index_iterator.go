...
type HistoryIndexIterator interface {
	...
	// the given number. Returns the ID and true boolean if the element with
	// the condition satisfied has been found, otherwise (0, false) is returned.
	SeekGT(id uint64) (uint64, bool, error)
...
// given number. Returns the ID and true boolean if the element with the condition
// satisfied has been found, otherwise (0, false) is returned.
//
// Note, this operation will unset the exhausted status and subsequent traversal
// is allowed.
func (it *blockIterator) SeekGT(id uint64) (uint64, bool, error) {
	var err error
	...
	for pos < limit {
		...
		if result > id {
			if pos == limit {
				it.set(pos, index, result)
			} else {
				it.set(pos, index-1, result)
			}
...
// Note, this operation will unset the exhausted status and subsequent traversal
// is allowed.
func (it *indexIterator) SeekGT(id uint64) (uint64, bool, error) {
