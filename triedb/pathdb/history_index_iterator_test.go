...
func checkSeekGT(it HistoryIndexIterator, input uint64, exp bool, expVal uint64) error {
	result, found, err := it.SeekGT(input)
	if err != nil {
		return err
	}
	if !exp {
		if found {
			return fmt.Errorf("unexpected returned value: %d", result)
		}
		return nil
	}
	if !found {
	...
	if result != expVal {
		return fmt.Errorf("unexpected returned value, want: %d, got: %d", expVal, result)
	}
	return nil
}
