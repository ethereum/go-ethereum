...
import (
	...
	"github.com/dezzyboy/go-ethereum/accounts/usbwallet/trezor"
...
// or whether the AkoinCash app was not started on it.
...
// AkoinCash address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AkoinCashAddress)
	if _, err := w.trezorExchange(&trezor.AkoinCashGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.AkoinCashSignTx{
	...
	response := new(trezor.AkoinCashTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AkoinCashTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
