...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-acash/master/doc/ethapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and AkoinCash address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an AkoinCash transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an AkoinCash message following the EIP 712 specification
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "AkoinCash app in browser mode", w.failure
	}
	if w.offline() {
		return "AkoinCash app offline", w.failure
	}
	return fmt.Sprintf("AkoinCash app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the AkoinCash app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// AkoinCash app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the AkoinCash app's version, will fail prior to v1.0.2
...
// and returning the AkoinCash address located on that derivation path.
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the AkoinCash app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the AkoinCash wallet app running
...
// ledgerDerive retrieves the currently active AkoinCash address from a Ledger
...
//   AkoinCash address length | 1 byte
//   AkoinCash address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the AkoinCash hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an AkoinCash address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the AkoinCash signature and do a sanity validation
