...
func NewPrivateAccountAPI(e *Expanse) *PrivateAccountAPI {
...
func (api *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger *vm.LogConfig) (*ExecutionResult, error) {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.exp.ChainDb(), txHash)
	if tx == nil {
		return nil, fmt.Errorf("transaction %x not found", txHash)
	}
	block := api.exp.BlockChain().GetBlock(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := api.exp.BlockChain().GetBlock(block.ParentHash())
	if parent == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		if uint64(idx) < txIndex {
			vmenv := core.NewEnv(stateDb, api.config, api.exp.BlockChain(), msg, block.Header(), vm.Config{})
		...
		vmenv := core.NewEnv(stateDb, api.config, api.exp.BlockChain(), msg, block.Header(), vm.Config{Debug: true, Logger: *logger})
