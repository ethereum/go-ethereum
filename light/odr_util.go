...
func GetTransaction(ctx context.Context, odr OdrBackend, txHash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error) {
	...
	// First ensure that we have the header, otherwise block body retrieval will
	// fail. Also verify if this is a canonical block by getting the header by
	// number and checking its hash.
