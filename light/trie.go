...
func (t *odrTrie) TryGetStorage(_, key []byte) ([]byte, error) {
...
func (t *odrTrie) TryUpdateStorage(_, key, value []byte) error {
	key = crypto.Keccak256(key)
	return t.do(key, func() error {
		return t.trie.TryUpdate(key, value)
	})
...
func (t *odrTrie) TryDeleteStorage(_, key []byte) error {
	key = crypto.Keccak256(key)
	return t.do(key, func() error {
