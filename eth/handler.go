...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	h.downloader = downloader.New(h.checkpointNumber, config.Database, stateBloom, h.eventMux, h.chain, nil, h.removePeer)
	...
	h.blockFetcher = fetcher.NewBlockFetcher(h.chain.GetBlockByHash, validator, h.BroadcastBlock, heighter, inserter, h.removePeer)
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkid.NewID(h.chain), h.forkFilter); err != nil {
