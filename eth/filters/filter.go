...
func (f *Filter) indexedLogs(ctx context.Context, chainView *filtermaps.ChainView, indexView *filtermaps.IndexView, begin, end uint64) ([]*types.Log, error) {
	...
	potentialLogs := make([]*types.Log, 0, len(potentialMatches))
	for _, logPosition := range potentialMatches {
		receipts := chainView.Receipts(logPosition.BlockNumber)
		if receipts == nil {
			err = fmt.Errorf("receipts for block #%d not found", logPosition.BlockNumber)
			break
		}
		log, err2 := logPosition.GetLog(receipts)
		if err2 != nil {
			err = err2
			break
		}
		if log != nil {
			potentialLogs = append(potentialLogs, log)
		}
	}
