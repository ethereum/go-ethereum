...
func New(stateDb ethdb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
	...
	return &Downloader{
		...
		headerProcCh:     make(chan []*types.Header, maxQueuedHeaders/(MaxHeaderFetch*MaxSkeletonSize)),
	}
}
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	case p.version == 61:
		...
		return d.spawnSync(origin+1, td,
...
func (d *Downloader) spawnSync(origin uint64, td *big.Int, fetchers ...func() error) error {
	var wg sync.WaitGroup
	errc := make(chan error, len(fetchers)+2)
	wg.Add(len(fetchers) + 2)
	go func() { defer wg.Done(); errc <- d.processHeaders(origin, td) }()
		...
		deliver = func(packet dataPack) (int, error) {
			...
			return d.queue.DeliverNodeData(packet.PeerId(), packet.(*statePack).states, func(err error, delivered int) {
				...
				if d.queue.PendingNodeData() > 0 {
					...
					// Peer gave us nothing, stalling fast sync, drop
