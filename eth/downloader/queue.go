...
func (q *queue) DeliverHeaders(id string, headers []*types.Header) (int, error) {
	...
	accepted := len(headers) == MaxHeaderFetch && headers[0].Number.Uint64() == request.From && headers[len(headers)-1].Hash() == target
	if accepted {
	...
	if !accepted {
		glog.V(logger.Detail).Infof("Peer %d: skeleton filling from header #%d not accepted", id, request.From)
