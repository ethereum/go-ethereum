...
type fetchResult struct {
	...
	AccessList   rlp.RawValue // EIP-7928 block access list, RLP encoded (ETH71)
	Withdrawals  types.Withdrawals
}

func newFetchResult(header *types.Header, snapSync bool, balCutoff uint64) *fetchResult {
	item := &fetchResult{
		Header: header,
	}
	if !header.EmptyBody() {
	...
	// Schedule the block access list (EIP-7928) for retrieval over ETH71 when the
	// header advertises one and it still falls within the window of blocks for
	// which remote peers are expected to serve access lists. Anything below the
	// cutoff is considered unavailable and is never requested. Access lists are
	// fetched in both full and snap sync: in full sync the downloaded list lets
	// the importer take the access-list-guided execution path, in snap sync it is
	// persisted directly alongside the snap-imported block.
	if header.BlockAccessListHash != nil && header.Number.Uint64() >= balCutoff {
...
type queue struct {
	...
	// balCutoff is the lowest block number for which block access lists are still
	// expected to be available from the network. Headers below this number never
	// have their access lists scheduled for retrieval (EIP-7928 / ETH71).
	balCutoff uint64
...
// PendingBALs retrieves the number of block access list requests pending for
// retrieval.
...
// InFlightBALs retrieves whether there are block access list fetch requests
// currently in flight.
...
func (q *queue) Schedule(headers []*types.Header, hashes []common.Hash, from uint64) int {
	...
	for i, header := range headers {
		...
		// Queue for block access list retrieval (EIP-7928 / ETH71). Access lists
		// are fetched in both full and snap sync, but only when the header carries
		// one and only for blocks recent enough that peers still serve them.
		if header.BlockAccessListHash != nil && header.Number.Uint64() >= q.balCutoff {
...
func (q *queue) Results(block bool) []*fetchResult {
	...
	for _, result := range results {
		...
		size += common.StorageSize(len(result.AccessList))
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque[int64, *types.Header],
	...
	for len(send) < count && !taskQueue.Empty() {
		...
		stale, throttle, item, err := q.resultCache.AddFetch(header, q.mode == ethconfig.SnapSync, q.balCutoff)
		if stale {
...
// queue. The method returns the number of access lists accepted from the
// delivery and also wakes any threads waiting for data delivery.
//
// The access lists are validated against the BlockAccessListHash advertised in
// the corresponding header, using the keccak256 digests precomputed by the eth
// protocol handler (the response metadata).
func (q *queue) DeliverBALs(id string, balList []eth.RawBlockAccessList, balHashes []common.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	reconstruct := func(index int, result *fetchResult) {
		// Copy out the canonical RLP encoding; the response buffer is owned by
		// the protocol layer and must not be retained.
		raw := balList[index].Bytes()
		encoded := make([]byte, len(raw))
		copy(encoded, raw)
		result.AccessList = encoded
		result.SetBALDone()
	}
	return q.deliver(id, q.balTaskPool, q.balTaskQueue, q.balPendPool,
		balReqTimer, balInMeter, balDropMeter, len(balList), validate, reconstruct)
}
...
//
// balCutoff is the lowest block number for which block access lists are still
// expected to be retrievable from the network; access lists for older blocks
// are treated as unavailable and never scheduled.
func (q *queue) Prepare(offset uint64, mode SyncMode, balCutoff uint64) {
	q.lock.Lock()
	defer q.lock.Unlock()

	// Prepare the queue for sync results
	q.resultCache.Prepare(offset)
	q.balCutoff = balCutoff
	q.mode = mode
}
