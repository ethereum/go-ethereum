...
func handleReceipts(backend Backend, msg Decoder, peer *Peer) error {
	...
	for i := range res.ReceiptsResponse {
		res.ReceiptsResponse[i].setBuffers(buffers)
	}
	...
	metadata := func() interface{} {
		...
		for i := range res.ReceiptsResponse {
			hashes[i] = types.DeriveSha(res.ReceiptsResponse[i], hasher)
		}
		return hashes
	}
	var enc ReceiptsRLPResponse
	for i := range res.ReceiptsResponse {
		enc = append(enc, res.ReceiptsResponse[i].EncodeForStorage())
	}
	return peer.dispatchResponse(&Response{
		id:   res.RequestId,
		code: ReceiptsMsg,
