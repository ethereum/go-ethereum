...
func TestEmptyMessages(t *testing.T) {
	...
	for i, msg := range []any{
		...
		ReceiptsPacket{1111, []*ReceiptList{}},
		// Transactions
...
func TestMessages(t *testing.T) {
	...
	// init the receipts
	{
		...
		rlpData, err := rlp.EncodeToBytes(NewReceiptList(receipts))
		if err != nil {
			t.Fatal(err)
		}
		receiptsRlp = rlpData
	...
	for i, tc := range []struct {
		...
		{
			ReceiptsPacket{1111, []*ReceiptList{NewReceiptList(receipts)}},
		...
		{
			// Identical to the eth/69 encoding above.
