...
func ServiceGetStorageRangesQuery(chain *core.BlockChain, req *GetStorageRangesPacket) ([][]*StorageData, [][]byte) {
	...
	for _, account := range req.Accounts {
		...
		if origin != (common.Hash{}) || (abort && len(storage) > 0) {
			...
			accTrie, err := trie.New(req.Root, common.Hash{}, req.Root, chain.StateCache().NodeDB())
			if err != nil {
				return nil, nil
			}
			...
			stTrie, err := trie.New(req.Root, account, acc.Root, chain.StateCache().NodeDB())
			if err != nil {
				return nil, nil
			}
			proof := light.NewNodeSet()
...
func ServiceGetTrieNodesQuery(chain *core.BlockChain, req *GetTrieNodesPacket, start time.Time) ([][]byte, error) {
	...
	accTrie, err := trie.NewSecure(req.Root, common.Hash{}, req.Root, chain.StateCache().NodeDB())
	if err != nil {
	...
	for _, pathset := range req.Paths {
		...
		default:
			...
			stTrie, err := trie.NewSecure(req.Root, common.BytesToHash(pathset[0]), common.BytesToHash(account.Root), chain.StateCache().NodeDB())
			loads++ // always account database reads, even for failures
			if err != nil {
				break
			}
