...
func ServiceGetAccountRangeQuery(chain *core.BlockChain, req *GetAccountRangePacket) ([]*AccountData, [][]byte) {
	...
	tr, err := trie.New(req.Root, common.Hash{}, req.Root, chain.StateCache().TrieDB())
	if err != nil {
		return nil, nil
	}
...
func ServiceGetStorageRangesQuery(chain *core.BlockChain, req *GetStorageRangesPacket) ([][]*StorageData, [][]byte) {
	...
	for _, account := range req.Accounts {
		...
		if origin != (common.Hash{}) || (abort && len(storage) > 0) {
			...
			accTrie, err := trie.NewStateTrie(req.Root, common.Hash{}, req.Root, chain.StateCache().TrieDB())
			if err != nil {
				return nil, nil
			}
			...
			stTrie, err := trie.NewStateTrie(req.Root, account, acc.Root, chain.StateCache().TrieDB())
			if err != nil {
				return nil, nil
			}
			proof := light.NewNodeSet()
