...
// PublicEthereumAPI provides an API to access Expanse related information.
...
// NewPublicEthereumAPI creates a new Expanse protocol API.
func NewPublicEthereumAPI(e *Expanse) *PublicEthereumAPI {
...
// ProtocolVersion returns the current Expanse protocol version this node supports
...
func NewPublicMinerAPI(e *Expanse) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Expanse) *PrivateMinerAPI {
...
func NewPublicTxPoolAPI(e *Expanse) *PublicTxPoolAPI {
...
// PublicBlockChainAPI provides an API to access the Expanse blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
func NewPublicTransactionPoolAPI(e *Expanse) *PublicTransactionPoolAPI {
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.exp.BlockChain().Export(out); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.exp.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.exp.BlockChain().InsertChain(blocks); err != nil {
...
// of the Expanse service.
func NewPublicDebugAPI(exp *Expanse) *PublicDebugAPI {
	return &PublicDebugAPI{exp: exp}
}
...
func (api *PrivateDebugAPI) ChaindbProperty(property string) (string, error) {
	ldb, ok := api.exp.chainDb.(interface {
		LDB() *leveldb.DB
	})
	if !ok {
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.Config) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.exp.BlockChain().GetBlock(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) SetHead(number uint64) {
	api.exp.BlockChain().SetHead(number)
}
...
func (s *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger *vm.LogConfig) (*ExecutionResult, error) {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(s.exp.ChainDb(), txHash)
	if tx == nil {
		return nil, fmt.Errorf("transaction %x not found", txHash)
	}
	block := s.exp.BlockChain().GetBlock(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := s.exp.BlockChain().GetBlock(block.ParentHash())
	if parent == nil {
		return nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	stateDb, err := state.New(parent.Root(), s.exp.ChainDb())
	if err != nil {
		return nil, err
	}
	...
	for idx, tx := range block.Transactions() {
		...
		if uint64(idx) < txIndex {
			vmenv := core.NewEnv(stateDb, s.config, s.exp.BlockChain(), msg, block.Header(), vm.Config{})
		...
		vmenv := core.NewEnv(stateDb, s.config, s.exp.BlockChain(), msg, block.Header(), vm.Config{Debug: true, Logger: *logger})
