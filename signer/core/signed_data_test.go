...
func TestGnosisSafe(t *testing.T) {
	...
	if sighash, rawdata, err := apitypes.TypedDataAndHash(typedData); err != nil {
		t.Fatal(err)
	} else if !bytes.Equal(sighash, gnosisTx.InputExpHash.Bytes()) {
		t.Logf("rawdata: %#x\n", rawdata)
		t.Fatal(fmt.Errorf("mismatched safeTxHash; have %#x want %#x", sighash, gnosisTx.InputExpHash[:]))
	}
}
