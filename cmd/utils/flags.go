...
var (
	...
	WhaleChainFlag = &cli.BoolFlag{
		Name:  "whalechain",
		Usage: "WhaleChain mainnet",
	}
	...
	WhaleChainTestnetV4Flag = &cli.BoolFlag{
		Name:  "whalechain-testnet-v4",
		Usage: "WhaleChain Testnet V4: pre-configured proof-of-stake test network",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(WhaleChainFlag.Name) {
			return filepath.Join(path, "whalechain")
		}
		...
		if ctx.Bool(WhaleChainTestnetV4Flag.Name) {
			return filepath.Join(path, "whalechain-testnet-v4")
		}
		return path
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(WhaleChainFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "whalechain")
	case ctx.Bool(GoerliFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "goerli")
	case ctx.Bool(SepoliaFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "sepolia")
	case ctx.Bool(WhaleChainTestnetV4Flag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "whalechain-testnet-v4")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.Bool(WhaleChainFlag.Name):
		...
		cfg.Genesis = core.DefaultWhaleChainGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.MainnetGenesisHash)
	case ctx.Bool(SepoliaFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
	...
	case ctx.Bool(WhaleChainTestnetV4Flag.Name):
		...
		cfg.Genesis = core.DefaultWhaleChainTestnetV4GenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.MainnetGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
