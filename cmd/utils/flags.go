...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 1=Frontier, 2=Morden (disused), 3=Ropsten, 4=Rinkeby, 314158=Stureby)",
	...
	SturebyFlag = cli.BoolFlag{
		Name:  "stureby",
		Usage: "Stureby network: pre-configured proof-of-work test network",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(SturebyFlag.Name) {
			return filepath.Join(path, "stureby")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(SturebyFlag.Name):
		urls = params.SturebyBootnodes
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(SturebyFlag.Name):
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "stureby")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	checkExclusive(ctx, DeveloperFlag, TestnetFlag, RinkebyFlag, SturebyFlag)
	...
	case ctx.GlobalBool(SturebyFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 314158
		}
		cfg.Genesis = core.DefaultSturebyGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(SturebyFlag.Name):
		genesis = core.DefaultSturebyGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
