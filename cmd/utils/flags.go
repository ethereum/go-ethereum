...
var (
	...
	UbqhashCachesInMemoryFlag = cli.IntFlag{
		...
		Value: eth.DefaultConfig.UbqhashCachesInMem,
	}
	...
	UbqhashDatasetsInMemoryFlag = cli.IntFlag{
		...
		Value: eth.DefaultConfig.UbqhashDatasetsInMem,
	}
	UbqhashDatasetsOnDiskFlag = cli.IntFlag{
		...
		Value: eth.DefaultConfig.UbqhashDatasetsOnDisk,
	}
...
func setUbqhash(ctx *cli.Context, cfg *eth.Config) {
	if ctx.GlobalIsSet(UbqhashCacheDirFlag.Name) {
		cfg.UbqhashCacheDir = ctx.GlobalString(UbqhashCacheDirFlag.Name)
	}
	if ctx.GlobalIsSet(UbqhashDatasetDirFlag.Name) {
		cfg.UbqhashDatasetDir = ctx.GlobalString(UbqhashDatasetDirFlag.Name)
	}
	if ctx.GlobalIsSet(UbqhashCachesInMemoryFlag.Name) {
		cfg.UbqhashCachesInMem = ctx.GlobalInt(UbqhashCachesInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(UbqhashCachesOnDiskFlag.Name) {
		cfg.UbqhashCachesOnDisk = ctx.GlobalInt(UbqhashCachesOnDiskFlag.Name)
	}
	if ctx.GlobalIsSet(UbqhashDatasetsInMemoryFlag.Name) {
		cfg.UbqhashDatasetsInMem = ctx.GlobalInt(UbqhashDatasetsInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(UbqhashDatasetsOnDiskFlag.Name) {
		cfg.UbqhashDatasetsOnDisk = ctx.GlobalInt(UbqhashDatasetsOnDiskFlag.Name)
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	if config.Clique != nil {
		...
		if !ctx.GlobalBool(FakePoWFlag.Name) {
			engine = ubqhash.New(
				stack.ResolvePath(eth.DefaultConfig.UbqhashCacheDir), eth.DefaultConfig.UbqhashCachesInMem, eth.DefaultConfig.UbqhashCachesOnDisk,
				stack.ResolvePath(eth.DefaultConfig.UbqhashDatasetDir), eth.DefaultConfig.UbqhashDatasetsInMem, eth.DefaultConfig.UbqhashDatasetsOnDisk,
			)
		}
	}
