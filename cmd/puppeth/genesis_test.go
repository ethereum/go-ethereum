...
func TestParityYoloConverter(t *testing.T) {
	...
	// There exists a reference config in 'yolo_parity.json'. The
	// file 'yolo_parity_generated.json' is generated by us, and have gone through
	// a json-diffing to see if it seems to match 'yolo_parity.json'. (seems ok).
	// We'll compare against that one, since it's a PITA to resolve minor json-differences
	expBlob, err := ioutil.ReadFile("testdata/yolo_parity_generated.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	if !bytes.Equal(expBlob, enc) {
