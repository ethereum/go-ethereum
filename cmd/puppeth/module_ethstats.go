...
RUN \
  apk add --update git                                         && \
  git clone --depth=1 https://github.com/karalabe/brn-netstats && \
	apk del git && rm -rf /var/cache/apk/*                       && \
	\
  cd /brn-netstats && npm install && npm install -g grunt-cli && grunt

WORKDIR /brn-netstats
EXPOSE 3000
...
services:
  brnstats:
    build: .
    image: {{.Network}}/brnstats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string) ([]byte, error) {
	...
	// Build and deploy the brnstats service
