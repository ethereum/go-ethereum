...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 50405, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 50405, peersTotal: 50, peersLight: 0, gasTarget: 7.5, gasLimit: 10, gasPrice: 1}
		}
	}
	existed := err == nil
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an ethbase to mine against
			fmt.Println()
			if infos.ethbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ethbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.ethbase)
				infos.ethbase = w.readDefaultAddress(common.HexToAddress(infos.ethbase)).Hex()
			}
