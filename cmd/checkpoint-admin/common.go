...
func newClient(ctx *cli.Context) *gclient.Client {
	client, err := gclient.Dial(ctx.String(nodeURLFlag.Name))
	if err != nil {
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, gclient.NewClient(client))
	if err != nil {
