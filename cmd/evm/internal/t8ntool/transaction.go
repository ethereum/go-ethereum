...
func Transaction(ctx *cli.Context) error {
	...
	for it.Next() {
		...
		if gas, err := core.IntrinsicGas(tx.Data(), tx.AccessList(), tx.AuthList(), tx.To() == nil,
			chainConfig.IsHomestead(new(big.Int)), chainConfig.IsIstanbul(new(big.Int)), chainConfig.IsShanghai(new(big.Int), 0), chainConfig.IsPrague(new(big.Int), 0)); err != nil {
			r.Error = err
			results = append(results, r)
			continue
		} else {
