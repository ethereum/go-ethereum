...
func generateEndpoint(scheme string, cluster string, app string, endpoint string) {
	if cluster == "prod" {
		endpoint = fmt.Sprintf("%s://%v.swarm-gateways.net", scheme)
	} else {
		endpoint = fmt.Sprintf("%s://%s-%v-%s.stg.swarm-gateways.net", scheme, app, cluster)
	}
}
...
func uploadSpeed(c *cli.Context) error {
	...
	generateEndpoints(scheme, cluster, appName, endpoint)
	...
	if single {
		...
		go func(endpoint string, ruid string) {
			for {
				...
				metrics.GetOrRegisterMeter("upload-speed.single.fetch-time", nil).Mark(int64(fetchTime))
				wg.Done()
				return
			}
		...
		for _, endpoint := range endpoints {
			...
			go func(endpoint string, ruid string) {
				for {
					...
					metrics.GetOrRegisterMeter("upload-speed.each.fetch-time", nil).Mark(int64(fetchTime))
					wg.Done()
					return
				}
			}(endpoint, ruid)
...
func fetch(hash string, endpoint string, original []byte, ruid string) error {
	ctx, sp := spancontext.StartSpan(context.Background(), "upload-speed.fetch")
	defer sp.Finish()
	...
	trace := client.GetClientTrace("upload-speed - http get", "upload-speed", ruid, &tn)
