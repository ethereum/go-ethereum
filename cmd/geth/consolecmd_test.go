...
func runMinimalGeth(t *testing.T, args ...string) *testgeth {
	// --sepolia to make the 'writing genesis to disk' faster (no accounts)
	// --networkid=1337 to avoid cache bump
	// --syncmode=full to avoid allocating fast sync bloom
	allArgs := []string{"--sepolia", "--networkid", "1337", "--authrpc.port", "0", "--syncmode=full", "--port", "0",
...
func TestConsoleWelcome(t *testing.T) {
	...
	geth.SetTemplateFunc("niltime", func() string {
		return time.Unix(int64(core.DefaultSepoliaGenesisBlock().Timestamp), 0).Format("Mon Jan 02 2006 15:04:05 GMT-0700 (MST)")
	})
