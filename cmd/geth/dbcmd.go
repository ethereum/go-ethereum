...
WARNING: This is a low-level operation which may cause database corruption!`,
	...
	dbGetSlotsCmd = &cli.Command{
		...
		ArgsUsage: "<hex-encoded state root> <hex-encoded storage trie root> <hex-encoded start (optional)> <int max elements (optional)>",
		Flags: flags.Merge([]cli.Flag{
			utils.SyncModeFlag,
		}, utils.NetworkFlags, utils.DatabasePathFlags),
		Description: "This command looks up the specified database key from the database.",
...
func dbDumpTrie(ctx *cli.Context) error {
	...
	if storage, err = hexutil.Decode(ctx.Args().Get(1)); err != nil {
		log.Info("Could not decode the root", "error", err)
		return err
	}
	if ctx.NArg() >= 3 {
		if start, err = hexutil.Decode(ctx.Args().Get(2)); err != nil {
			log.Info("Could not decode the seek position", "error", err)
			return err
		}
	}
	if ctx.NArg() >= 4 {
		if max, err = strconv.ParseInt(ctx.Args().Get(3), 10, 64); err != nil {
			log.Info("Could not decode the max count", "error", err)
			return err
		}
	}
	theTrie, err := trie.New(common.BytesToHash(state), common.Hash{}, common.BytesToHash(storage), trie.NewDatabase(db))
	if err != nil {
		return err
	}
	var count int64
