...
func (te *test.testenv) checkPong(reply v4wire.Packet, pingHash []byte) error {
...
func PingWrongTo(t *testing.T) {
	te := test.newTestEnv(*remote, *waitTime)
	defer te.close()
...
func UnsolicitedNeighbors(t *testing.T) {
	...
	if test.contains(nodes, encFakeKey) {
...
func bond(t *testing.T, te *test.testenv) {
	ping := v4wire.Ping{
		Version:    4,
		From:       te.localEndpoint(te.l1),
		To:         te.remoteEndpoint(),
