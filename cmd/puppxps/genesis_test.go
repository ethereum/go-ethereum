...
// Tests the go-xpayments to Alxps chainspec conversion for the Stureby testnet.
func TestAlxpsSturebyConverter(t *testing.T) {
	...
	spec, err := newAlxpsGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alxps.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alxpsGenesisSpec{}
