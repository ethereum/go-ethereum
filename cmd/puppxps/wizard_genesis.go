...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alxps.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the genesis spec used by Alxps (formerly C++ xPayments)
		if spec, err := newAlxpsGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Aleth chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alxps", spec)
		}
