...
// alxpsGenesisSpec represents the genesis specification format used by the
// C++ xPayments implementation.
type alxpsGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alxpsGenesisSpecAccount `json:"accounts"`
}

// alxpsGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alxpsGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alxpsGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alxpsGenesisSpecBuiltin is the precompiled contract definition.
type alxpsGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *alxpsGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alxpsGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlxpsGenesisSpec converts a go-xpayments genesis block into a Alxps-specific
// chain specification format.
func newAlxpsGenesisSpec(network string, genesis *core.Genesis) (*alxpsGenesisSpec, error) {
	// Only xpsash is currently supported between go-xpayments and alxps
	if genesis.Config.Xpsash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alxps format
	spec := &alxpsGenesisSpec{
		SealEngine: "Xpsash",
	}
	// Some defaults
	...
	// config but alxps will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.setPrecompile(1, &alxpsGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alxpsGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alxpsGenesisSpecBuiltin{Name: "sha256",
		Linear: &alxpsGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alxpsGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alxpsGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alxpsGenesisSpecBuiltin{Name: "identity",
		Linear: &alxpsGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alxpsGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &alxpsGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alxpsGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alxpsGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alxpsGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alxpsGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &alxpsGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alxps hardcoded the gas policy
		spec.setPrecompile(7, &alxpsGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alxps hardcoded the gas policy
		spec.setPrecompile(9, &alxpsGenesisSpecBuiltin{
...
func (spec *alxpsGenesisSpec) setPrecompile(address byte, data *alxpsGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alxpsGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alxpsGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alxpsGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alxpsGenesisSpecAccount)
	}
