...
import (
	...
	"github.com/chattynet/chatty/rpc"
...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `chatty Monitor: node metrics monitoring and visualization`,
		Description: `
The chatty monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) {
	...
	// Attach to an chatty node over IPC or RPC
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by chatty (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached chatty node and retrieves the entire set
