...
// chatty is the official command-line client for Ethereum.
package main
...
func init() {
	...
	app = utils.NewApp(Version, "the Chatty command line interface")
	app.Action = run
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print chatty version numbers",
			Description: `
...
`,
		...
		{
			Name:  "wallet",
			Usage: "chatty presale wallet",
			Subcommands: []cli.Command{
				{
					Action: importWallet,
					Name:   "import",
					Usage:  "Not used in Chatty.",
				},
			},
			Description: `
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    chatty --password <passwordfile> account new
...
// doesn't work since it's in memory for the running instance: TODO: do an IPC call to it
    ...
    chatty account locked <address>

Check locked status of an existing account.

chatty account locked <addr>
					`,
				},*/
				{
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    chatty --password <passwordfile> account import <keyfile>
...
nodes.
		...
		},
		{
			Action: console,
			Name:   "console",
			Usage:  `Chatty Console: interactive JavaScript environment`,
			Description: `
The chatty console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/chattynet/chatty/wiki/Javascipt-Console
`},
		{
			Action: attach,
			Name:   "attach",
			Usage:  `Chatty Console: interactive JavaScript environment (connect to node)`,
			Description: `
The chatty console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/chattynet/chatty/wiki/Javascipt-Console.
This command allows to open a console on a running chatty node.
`,
		},
		{
			Action: execJSFiles,
			Name:   "js",
			Usage:  `executes the given JavaScript files in the chatty JavaScript VM`,
			Description: `
The JavaScript VM exposes a node admin interface as well as the Ðapp
JavaScript API. See https://github.com/chattynet/chatty/wiki/Javascipt-Console
`,
		},
	}
	app.Flags = []cli.Flag{
...
func attach(ctx *cli.Context) {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to chatty node - %v", err)
	}
...
func makedag(ctx *cli.Context) {
	...
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: chatty makedag <block number> <outputdir>`)
	}
	switch {
	case len(args) == 2:
