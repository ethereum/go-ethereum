...
class AccessLists {
        ...
        if ((0, types_js_1.isAccessList)(accessList)) {
            ...
            for (let i = 0; i < accessList.length; i++) {
                ...
                for (let index = 0; index < item.storageKeys.length; index++) {
                    storageItems.push((0, ethereumjs_util_1.hexToBytes)(item.storageKeys[index]));
                }
            ...
            for (let i = 0; i < bufferAccessList.length; i++) {
                ...
                const address = (0, ethereumjs_util_1.bytesToHex)(data[0]);
                const storageKeys = [];
                for (let item = 0; item < data[1].length; item++) {
                    storageKeys.push((0, ethereumjs_util_1.bytesToHex)(data[1][item]));
                }
                const jsonItem = {
                    address,
                    storageKeys,
    ...
    static getAccessListJSON(accessList) {
        ...
        for (let index = 0; index < accessList.length; index++) {
            ...
            const JSONItem = {
                address: (0, ethereumjs_util_1.bytesToHex)((0, ethereumjs_util_1.setLengthLeft)(item[0], 20)),
                storageKeys: [],
            };
            const storageSlots = item[1];
            for (let slot = 0; slot < storageSlots.length; slot++) {
                const storageSlot = storageSlots[slot];
                JSONItem.storageKeys.push((0, ethereumjs_util_1.bytesToHex)((0, ethereumjs_util_1.setLengthLeft)(storageSlot, 32)));
            }
...
function txTypeBytes(txType) {
    return (0, ethereumjs_util_1.hexToBytes)('0x' + txType.toString(16).padStart(2, '0'));
}
