...
This is a post-mortem concerning the minority split that occurred on Ethereum mainnet on block TODO, at 2021-08-22 TODO UTC
...
Since we had merged the removal of `ETH65`, if the entire network were to upgrade, then nodes which have not yet implemented `ETH66` would be cut off from the network. After further discussions, it was decided to
