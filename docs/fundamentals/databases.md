...
Older segments of the chain are moved out of the fast LevelDB database and into a freezer database the is less performant. Nodes rarely need to access these files so IO speed is less important and the bulk of the chain data can be stored on a cheaper HDD. Once blocks pass some threshold age (128 blocks behind the head by default) the block and receipt data is flattened and saved as a raw binary blob of data along with an index entry file used for identification. This flat file format allows Geth to capitalize on the operating system's built-in memory manager and file cacher which in turn allows Geth to maximize the available memory without hogging it from other processes. This is good for overall system stability and also reduces the strain on Go's garbage collector and reduces the total amount of memory used by Geth. 
