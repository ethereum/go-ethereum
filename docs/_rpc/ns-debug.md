...
The `debug_traceCall` method lets you run an `eth_call` within a given block using the parent block state as the base. The block can be specified either by hash or by number. It takes the same input object as a `eth_call`.
