...
Geth code is available to IoS and Android mobile developers. This relies on Geth running a 
[light client](/docs/interface/les) on the mobile device and exposing an API that developers 
can use to build mobiule apps on top of Geth. However, since Ethereum light clients are 
currently too heavy to run on most mobile devices, this is should be considered a proof-of-concept 
project for now. This page explains how to download Geth for mobile and how to manage Ethereum 
accounts in mobile applications.
...
These bundles can be downloaded directly from [our website](https://geth.ethereum.org/downloads/) 
...
The Geth mobile API is generally equivalent to the [Go API](/docs/dapp/native).

## Mobile Account Management

Best practise for account management is to do it client-side, with all sensitive information
self-contained inside the local application. This ensures the mobile developer/user retains
fine-grained control over the access permissions to user-data instead of outsourcing security
to a third party.
