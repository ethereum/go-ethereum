...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, keys [][]byte, values [][]byte, firstProof ethdb.KeyValueReader, lastProof ethdb.KeyValueReader) (ethdb.KeyValueStore, *Trie, bool, error) {
	if len(keys) != len(values) {
		return nil, nil, false, fmt.Errorf("inconsistent proof data, keys: %d, values: %d", len(keys), len(values))
	}
	if len(keys) == 0 {
		return nil, nil, false, errors.New("empty proof")
	}
	...
	// Special case, there is only one element and left edge proof is existent
	if len(keys) == 1 && bytes.Equal(keys[0], firstKey) {
		...
		if !bytes.Equal(val, values[0]) {
			return nil, nil, false, fmt.Errorf("correct proof but invalid data")
		}
		return nil, nil, hasRightElement(root, keys[0]), nil // TODO(karalabe): initialize the returned db
	}
