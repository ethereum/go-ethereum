...
func TestEmptySync(t *testing.T) {
	dbA := NewDatabase(keyvalue.NewMemoryDatabase())
	dbB := NewDatabase(keyvalue.NewMemoryDatabase())
	emptyA, _ := New(common.Hash{}, dbA)
	emptyB, _ := New(emptyRoot, dbB)

	for i, trie := range []*Trie{emptyA, emptyB} {
		if req := NewSync(trie.Hash(), keyvalue.NewMemoryDatabase(), nil).Missing(1); len(req) != 0 {
