...
func NewVerkleTrie(root verkle.VerkleNode, diskdb ethdb.KeyValueStore, pointCache *utils.PointCache) *VerkleTrie {
	return &VerkleTrie{
		root:       root,
		diskdb:     diskdb,
...
func (trie *VerkleTrie) GetStorage(addr common.Address, key []byte) ([]byte, error) {
	...
	return trie.root.Get(k, trie.diskdb.Get)
}
...
func (trie *VerkleTrie) GetWithHashedKey(key []byte) ([]byte, error) {
	return trie.root.Get(key, trie.diskdb.Get)
}
...
func (trie *VerkleTrie) UpdateStorage(addr common.Address, key, value []byte) error {
	k := utils.GetTreeKeyStorageSlotWithEvaluatedAddress(trie.pointCache.GetTreeKeyHeader(addr[:]), key)
	var v [32]byte
	copy(v[:], value[:])
...
func (trie *VerkleTrie) Commit(_ bool) (common.Hash, *NodeSet) {
	...
	for _, node := range nodes {
		if err := trie.diskdb.Put(node.CommitmentBytes[:], node.SerializedBytes); err != nil {
			return common.Hash{}, nil
		}
	}

	return nodes[0].CommitmentBytes, nil
}
...
func (trie *VerkleTrie) Copy(diskdb ethdb.KeyValueStore) *VerkleTrie {
	return &VerkleTrie{
		root:   trie.root.Copy(),
		diskdb: diskdb,
	}
