...
func (trie *VerkleTrie) DeleteStorage(addr common.Address, key []byte) error {
	pointEval := trie.pointCache.GetTreeKeyHeader(addr[:])
	k := utils.GetTreeKeyStorageSlotWithEvaluatedAddress(pointEval, key)
	_, err := trie.root.Delete(k, func(h []byte) ([]byte, error) {
		return trie.db.diskdb.Get(h)
	})
	return err
}
...
func (trie *VerkleTrie) Commit(_ bool) (common.Hash, *trienode.NodeSet) {
...
func (trie *VerkleTrie) Prove(_ []byte, _ ethdb.KeyValueWriter) error {
