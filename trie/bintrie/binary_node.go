...
func deserializeSubtree(remainingDepth int, position int, nodeDepth int, bitmap []byte, hashData []byte, hashIdx *int, mustRecompute bool) (BinaryNode, error) {
	if remainingDepth == 0 {
	...
	// note that the parent might not need root recomputations, but the children
	// do, because their hash isn't saved. This will incur more hash recomputations
	...
	// will the children.
	left, err := deserializeSubtree(remainingDepth-1, leftPos, nodeDepth+1, bitmap, hashData, hashIdx, true)
	if err != nil {
		return nil, err
	}
	right, err := deserializeSubtree(remainingDepth-1, rightPos, nodeDepth+1, bitmap, hashData, hashIdx, true)
	if err != nil {
		return nil, err
	}
...
func deserializeNode(serialized []byte, depth int, hn common.Hash, mustRecompute bool) (BinaryNode, error) {
	...
	case nodeTypeInternal:
		...
		hashIdx := 0
		return deserializeSubtree(groupDepth, 0, depth, bitmap, hashData, &hashIdx, mustRecompute)
	case nodeTypeStem:
