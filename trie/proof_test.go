...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *keyvalue.MemoryDatabase {
	var provers []func(key []byte) *keyvalue.MemoryDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *keyvalue.MemoryDatabase {
		proof := keyvalue.NewMemoryDatabase()
...
func BenchmarkProve(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		proofs := keyvalue.NewMemoryDatabase()
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	root := trie.Hash()
	var keys []string
	var proofs []*keyvalue.MemoryDatabase
	for k := range vals {
		keys = append(keys, k)
		proof := keyvalue.NewMemoryDatabase()
