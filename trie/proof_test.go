...
func TestRangeProof(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		...
		_, _, err := VerifyRangeProof(trie.Hash(), keys[0], keys, vals, firstProof, lastProof)
		if err != nil {
...
func TestRangeProofWithNonExistentProof(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		...
		_, _, err := VerifyRangeProof(trie.Hash(), first, keys, vals, firstProof, lastProof)
		if err != nil {
			t.Fatalf("Case %d(%d->%d) expect no error, got %v", i, start, end-1, err)
		}
	}
...
func TestRangeProofWithInvalidNonExistentProof(t *testing.T) {
	...
	_, _, err := VerifyRangeProof(trie.Hash(), first, k, v, firstProof, lastProof)
	if err == nil {
...
func TestOneElementRangeProof(t *testing.T) {
	...
	_, _, err := VerifyRangeProof(trie.Hash(), entries[start].k, [][]byte{entries[start].k}, [][]byte{entries[start].v}, firstProof, lastProof)
	if err != nil {
	...
	_, _, err = VerifyRangeProof(trie.Hash(), first, [][]byte{entries[start].k}, [][]byte{entries[start].v}, firstProof, lastProof)
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
}
...
func TestAllElementsProof(t *testing.T) {
	...
	_, _, err := VerifyRangeProof(trie.Hash(), k[0], k, v, nil, nil)
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	...
	_, _, err = VerifyRangeProof(trie.Hash(), k[0], k, v, firstProof, lastProof)
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
}
...
func TestSingleSideRangeProof(t *testing.T) {
	for i := 0; i < 64; i++ {
		...
		for _, pos := range cases {
			...
			_, _, err := VerifyRangeProof(trie.Hash(), common.Hash{}.Bytes(), k, v, firstProof, lastProof)
			if err != nil {
				t.Fatalf("Expected no error, got %v", err)
			}
		}
...
func TestHasRightElement(t *testing.T) {
	...
	for _, c := range cases {
		...
		hasMore, _, err := VerifyRangeProof(trie.Hash(), firstKey, k, v, firstProof, lastProof)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
...
func TestBloatRangeProof(t *testing.T) {
	trie := new(Trie)
	var entries entrySlice
	for i := 0; i < 4096; i++ {
		value := &kv{randBytes(32), randBytes(20), false}
	...
	var cases = []struct {
		...
		{-1, 1}, // single element with non-existent left proof
		{0, 1},  // single element with existent left proof
		{0, 10},
		{50, 100},
		{50, len(entries)},               // No more element expected
		{len(entries) - 1, len(entries)}, // Single last element
		{0, len(entries)},                // The whole set with existent left proof
		{-1, len(entries)},               // The whole set with non-existent left proof
	}
	for _, c := range cases {
		var (
			firstKey   []byte
		...
		_, resolved, err := VerifyRangeProof(trie.Hash(), firstKey, k, v, proof, proof)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		if proof.Len() != resolved {
			t.Fatalf("Bloating range proof, want %d, got %d", proof.Len(), resolved)
		}
	}
}
...
func benchmarkVerifyRangeProof(b *testing.B, size int) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, err := VerifyRangeProof(trie.Hash(), keys[0], keys, values, firstProof, lastProof)
		if err != nil {
