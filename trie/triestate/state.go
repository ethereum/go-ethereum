...
type Set struct {
	...
	RawSlotKey bool                                      // Flag whether the storage set uses the raw slot key or the hash of it
	size       common.StorageSize                        // Approximate size of set
}

// New constructs the state set with provided data.
func New(accounts map[common.Address][]byte, storages map[common.Address]map[common.Hash][]byte, rawSlotKey bool) *Set {
	return &Set{
		Accounts:   accounts,
		Storages:   storages,
