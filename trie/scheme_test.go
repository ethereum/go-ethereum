...
func TestEncodeNodeKey(t *testing.T) {
	...
	var cases = []struct {
		...
		{common.HexToHash(""), []byte{16}, []byte{0x20}},

		// no owner, odd length, no terminator
		{common.HexToHash(""), []byte{1, 2, 3, 4, 5}, []byte{0x11, 0x23, 0x45}},
		// no owner, even length, no terminator
		{common.HexToHash(""), []byte{0, 1, 2, 3, 4, 5}, []byte{0x00, 0x01, 0x23, 0x45}},
		// no owner, odd length, terminator
		{common.HexToHash(""), []byte{15, 1, 12, 11, 8, 16 /*term*/}, []byte{0x3f, 0x1c, 0xb8}},
		// no owner, even length, terminator
		{common.HexToHash(""), []byte{0, 15, 1, 12, 11, 8, 16 /*term*/}, []byte{0x20, 0x0f, 0x1c, 0xb8}},

		// with owner, empty keys, with and without terminator.
		{randomOwner, []byte{}, append(randomOwner.Bytes(), []byte{0x00}...)},
		{randomOwner, []byte{16}, append(randomOwner.Bytes(), []byte{0x20}...)},
		// with owner, odd length, no terminator
		{randomOwner, []byte{1, 2, 3, 4, 5}, append(randomOwner.Bytes(), []byte{0x11, 0x23, 0x45}...)},
		// with owner, even length, no terminator
		{randomOwner, []byte{0, 1, 2, 3, 4, 5}, append(randomOwner.Bytes(), []byte{0x00, 0x01, 0x23, 0x45}...)},
		// with owner, odd length, terminator
		{randomOwner, []byte{15, 1, 12, 11, 8, 16 /*term*/}, append(randomOwner.Bytes(), []byte{0x3f, 0x1c, 0xb8}...)},
