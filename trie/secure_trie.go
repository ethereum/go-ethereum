...
func (t *StateTrie) Get(key []byte) []byte {
	res, err := t.TryGetStorage(nil, key)
	if err != nil {
...
func (t *StateTrie) TryGetStorage(_, key []byte) ([]byte, error) {
...
func (t *StateTrie) Update(key, value []byte) {
	if err := t.TryUpdateStorage(nil, key, value); err != nil {
...
func (t *StateTrie) TryUpdateStorage(_, key, value []byte) error {
	hk := t.hashKey(key)
...
func (t *StateTrie) Delete(key []byte) {
	if err := t.TryDeleteStorage(nil, key); err != nil {
...
// If a node is not found in the database, a MissingNodeError is returned.
func (t *StateTrie) TryDeleteStorage(_, key []byte) error {
	hk := t.hashKey(key)
