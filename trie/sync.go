...
type Sync struct {
	database database.Reader          // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database database.Reader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) Commit(dbw database.Writer) (int, error) {
