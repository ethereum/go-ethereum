...
func (n *cachedNode) memorySize(key int) int {
	return int(n.size) + cachedNodeSize + key
}
...
// the function in cachedNode by counting the size of previous value.
func (n *nodeWithPreValue) memorySize(key int) int {
	return n.cachedNode.memorySize(key) + len(n.pre)
}
