...
func RotatingFileHandler(filePath string, limit uint64, formatter Format) (Handler, error) {
	...
	fileWriter, err := NewAsyncFileWriter(filePath, int64(limit))
	if err != nil {
		return nil, fmt.Errorf("could not create async file writer, %v", err)
	}
	fileWriter.Start()
