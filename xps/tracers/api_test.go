...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.Xpser)},
		accounts[1].addr: {Balance: big.NewInt(params.Xpser)},
		accounts[2].addr: {Balance: big.NewInt(params.Xpser)},
	}}
	genBlocks := 10
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &xpsapi.StateOverride{
					randomAccounts[0].addr: xpsapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Xpser)))},
				},
			},
