...
func TestRPCGetTransactionReceipt(t *testing.T) {
	...
	backend := newTestBackend(t, genBlocks, genesis, func(i int, b *core.BlockGen) {
		...
		case 2:
			...
			tx, _ = types.SignTx(types.NewTx(&types.LegacyTx{Nonce: uint64(i), To: &contract, Gas: 60000, GasPrice: b.BaseFee(), Data: common.FromHex(data)}), signer, acc1Key)
		}
		if tx != nil {
			b.AddTx(tx)
			txHashes[i] = tx.Hash()
