...
import (
	...
	"github.com/dezzyboy/go-ethereum/common/math"
...
// PublicAkoinCashAPI provides an API to access AkoinCash related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicAkoinCashAPI struct {
	b Backend
}

// NewPublicAkoinCashAPI creates a new AkoinCash protocol API.
func NewPublicAkoinCashAPI(b Backend) *PublicAkoinCashAPI {
	return &PublicAkoinCashAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicAkoinCashAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicAkoinCashAPI) Syncing() (interface{}, error) {
...
// Sign calculates an AkoinCash ECDSA signature for:
// keccack256("\x19AkoinCash Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/dezzyboy/go-ethereum/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19AkoinCash Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/dezzyboy/go-ethereum/wiki/Management-APIs#personal_ecRecover
...
// PublicBlockChainAPI provides an API to access the AkoinCash blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new AkoinCash blockchain API.
...
// PublicDebugAPI is the collection of AkoinCash APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of AkoinCash APIs exposed over the private
// debugging endpoint.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Acash)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f acash) exceeds the configured cap (%.2f acash)", feeFloat, cap)
	}
	return nil
}
