...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeXps := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Xpser)))
	feeFloat, _ := feeXps.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f xpser) exceeds the configured cap (%.2f xpser)", feeFloat, cap)
	}
	return nil
}
