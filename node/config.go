...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of aiigo is "aiigo". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Aiigo", keep that.
	if name == "aiigo" || name == "aiigo-testnet" {
		name = "Aiigo"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "aiigo" instances.
var isOldAiigoResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by aiigo 1.4 are used if they exist.
	if warn, isOld := isOldAiigoResource[path]; isOld {
		oldpath := ""
		if c.name() == "aiigo" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldAiigoResourceWarning {
				c.oldAiigoResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'aiigo' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
