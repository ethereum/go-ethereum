...
func TestAppendString(t *testing.T) {
	...
	for _, test := range tests {
		x := AppendString(test.slice, test.input)
		if !bytes.Equal(x, unhex(test.output)) {
			t.Errorf("TestAppendString(%v, %d): got %x, want %s", test.slice, test.input, x, test.output)
		}
	}
}
...
func TestEndList(t *testing.T) {
	...
	var encode func(buf []byte, v interface{}) []byte
