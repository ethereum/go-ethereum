...
func getChain() *core.BlockChain {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(gspec, gash.NewFaker(), 2, func(i int, gen *core.BlockGen) {})
	...
	bc, _ := core.NewBlockChain(rawdb.NewMemoryDatabase(), cacheConf, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
