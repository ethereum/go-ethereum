...
func makeTries() (chtTrie *trie.Trie, bloomTrie *trie.Trie, chtKeys, bloomKeys [][]byte) {
	chtTrie = trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase(), &trie.Config{Legacy: true}))
	bloomTrie = trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase(), &trie.Config{Legacy: true}))
