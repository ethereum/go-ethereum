...
exports.UNISWAP_V2_ROUTER_CONTRACT = void 0;
...
const hints = {
    swapETHForExactTokens(v, opt) {
        ...
        return `Swap up to ${utils_ts_1.weieth.encode(opt.amount)} ETH for exact ${last}. ${uniTs(v.deadline)}`;
    },
    swapExactETHForTokens(v, opt) {
        const last = uniToken(v.path, opt, v.amountOutMin, false);
        if (!opt.amount || !last)
            throw new Error('Not enough info');
        return `Swap ${utils_ts_1.weieth.encode(opt.amount)} ETH for at least ${last}. ${uniTs(v.deadline)}`;
    },
    ...
    swapExactTokensForETH(v, opt) {
        ...
        if (!first)
            throw new Error('Not enough info');
        return `Swap exact ${first} for at least ${utils_ts_1.weieth.encode(v.amountOutMin)} ETH. ${uniTs(v.deadline)}`;
    },
    ...
    swapTokensForExactETH(v, opt) {
        ...
        if (!first)
            throw new Error('Not enough info');
        return `Swap up to ${first} for exact ${utils_ts_1.weieth.encode(v.amountOut)} ETH. ${uniTs(v.deadline)}`;
    },
