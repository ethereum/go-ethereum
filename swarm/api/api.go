...
// NewAPI initialises the api constructor
func NewAPI(dpa *storage.DPA, dns Resolver) (a *API) {
	a = &API{
		dpa: dpa,
		dns: dns,
	}
...
// Upload is to be used only in TEST.
...
// Retrieve is the DPA reader API
...
func (a *API) Store(data io.Reader, size int64, wg *sync.WaitGroup) (key storage.Key, err error) {
	return a.dpa.Store(data, size, wg, nil)
}

type ErrResolve error

// Resolve resolves the DNS.
...
func (a *API) Put(content, contentType string) (storage.Key, error) {
	...
	wg := &sync.WaitGroup{}
	key, err := a.dpa.Store(r, int64(len(content)), wg, nil)
	if err != nil {
		apiPutFail.Inc(1)
		return nil, err
	}
	...
	key, err = a.dpa.Store(r, int64(len(manifest)), wg, nil)
	if err != nil {
		apiPutFail.Inc(1)
		return nil, err
	}
