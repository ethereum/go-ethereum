...
// NewDefaultStoreParams creates params with default values.
...
// have been evaluated
func (p *StoreParams) Init(path string) {
	p.ChunkDbPath = filepath.Join(path, "chunks")
}

// NewNetStore is a constructor and takes path argument that is used to initialise dbStore,
...
// Put stores logic common to local and network chunk store requests
...
// Get retrieves logic common for local and network chunk retrieval requests
