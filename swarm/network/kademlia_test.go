...
func testKademliaCase(t *testing.T, pivotAddr string, addrs ...string) {
	...
	h := pp.Healthy()
	if !(h.GotNN && h.KnowNN && h.Saturated) {
...
func TestKademliaCase3(t *testing.T) {
	t.Skip("depth definition has changed, causing this case to be invalid. A new batch of test cases are needed ASAP")
	testKademliaCase(t,
