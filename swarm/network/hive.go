...
// have been evaluated
func (p *HiveParams) Init(path string) {
	p.KadDbPath = filepath.Join(path, "bzz-peers.json")
}
...
func (h *Hive) keepAlive() {
	...
	for {
		peersNumGauge.Update(int64(h.kad.Count()))
		select {
		case <-alarm:
			if h.kad.DBCount() > 0 {
				select {
...
// HandlePeersMsg is called by the protocol when receiving peerset (for target address)
