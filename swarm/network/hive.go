	...
	go func() {
		...
		for more := range self.more {
			...
			if addr != nil {
				...
				connectPeer(NodeID(addr).String())
			}
...
func (self *Hive) handleGetPeersMsg(p Node) func(interface{}) error {
	return func(msg interface{}) error {
		req := msg.(*getPeersMsg)
		var peers []*peerAddr
		self.EachNode(p.RemoteOverlayAddr(), int(req.Order), func(n Node) bool {
			peers = append(peers, &peerAddr{n.RemoteOverlayAddr(), n.RemoteUnderlayAddr()})
...
func (self *Hive) PeerInfo(id discover.NodeID) interface{} {
	...
	return interface{}(&peerAddr{p.RemoteOverlayAddr(), p.RemoteUnderlayAddr()})
}
