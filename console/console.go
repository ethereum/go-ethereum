...
func (c *Console) init(preload []string) error {
	...
	flatten := "var brn = web3.brn; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = brn.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines brn.getBalance(brn.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + brn.coinbase);
		console.log("at block: " + brn.blockNumber + " (" + new Date(1000 * brn.getBlock(brn.blockNumber).timestamp) + ")");
