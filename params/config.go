...
func (c *ChainConfig) BlobConfig(ts uint64) *BlobConfig {
	switch {
	case c.BPO5Time != nil && ts >= *c.BPO5Time:
		return c.BlobScheduleConfig.BPO5
	case c.BPO4Time != nil && ts >= *c.BPO4Time:
		return c.BlobScheduleConfig.BPO4
	case c.BPO3Time != nil && ts >= *c.BPO3Time:
		return c.BlobScheduleConfig.BPO3
	case c.BPO2Time != nil && ts >= *c.BPO2Time:
		return c.BlobScheduleConfig.BPO2
	case c.BPO1Time != nil && ts >= *c.BPO1Time:
		return c.BlobScheduleConfig.BPO1
	case c.PragueTime != nil && ts >= *c.PragueTime:
		return c.BlobScheduleConfig.Prague
	case c.CancunTime != nil && ts >= *c.CancunTime:
