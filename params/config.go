...
type ChainConfig struct {
	...
	ShangaiBlock        *big.Int `json:"shangaiBlock,omitempty"`        // Shangai switch block (nil = no fork, 0 = already on shangai)
...
func (c *ChainConfig) String() string {
	...
	if c.ShangaiBlock != nil {
		banner += fmt.Sprintf(" - Shangai:                     %-8v (https://github.com/ethereum/execution-specs/blob/master/network-upgrades/mainnet-upgrades/shangai.md)\n", c.ShangaiBlock)
	}
...
// IsShangai returns whether num is either equal to the Shangai fork block or greater.
func (c *ChainConfig) IsShangai(num *big.Int) bool {
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "shangaiBlock", block: c.ShangaiBlock, optional: true},
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.ShangaiBlock, newcfg.ShangaiBlock, head) {
		return newCompatError("Shangai fork block", c.ShangaiBlock, newcfg.ShangaiBlock)
	}
...
type Rules struct {
	...
	IsMerge, IsShangai, isCancun                            bool
}
