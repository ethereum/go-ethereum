...
var (
	// Files that end up in the aiigo*.zip archive.
	aiigoArchiveFiles = []string{
		"COPYING",
		executablePath("aiigo"),
	}

	// Files that end up in the aiigo-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "aiigostore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		baseaiigo = archiveBasename(*arch, version.Archive(env.Commit))
		aiigo     = "aiigo-" + baseaiigo + ext
		alltools = "aiigo-alltools-" + baseaiigo + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(aiigo, aiigoArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{aiigo, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with aiigo-release.pub"
...
func doDockerBuildx(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, Aiigo only
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, Aiigo only
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, Aiigo & tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, Aiigo & tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Aiigo only
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Aiigo & tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Aiigo only
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Aiigo & tools
	var tags []string
	...
	for _, spec := range []struct {
		...
		for _, tag := range tags { // latest, stable etc
			aiigoImage := fmt.Sprintf("%s%s", spec.base, tag)
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "aiigo-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "aiigo-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "aiigo.exe" {
			aiigoTool = file
		} else {
	...
	// first section contains the aiigo binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Aiigo":     aiigoTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.aiigo.nsi", filepath.Join(*workdir, "aiigo.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("aiigo-" + archiveBasename(*arch, version.Archive(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DBUILDVERSION="+ver[2],
		"/DARCH="+*arch,
		filepath.Join(*workdir, "aiigo.nsi"),
	)
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "aiigostore/builds")`)
