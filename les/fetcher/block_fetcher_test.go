...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	if int(counter.Load()) != targetBlocks {
		t.Fatalf("retrieval count mismatch: have %v, want %v", counter.Load(), targetBlocks)
	}
	verifyChainHeight(t, tester, uint64(len(hashes)-1))
}
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	if int(counter.Load()) != 1 {
		t.Fatalf("retrieval count mismatch: have %v, want %v", counter.Load(), 1)
	}
...
func TestImportDeduplication(t *testing.T) {
	...
	if counter.Load() != 2 {
		t.Fatalf("import invocation count mismatch: have %v, want %v", counter.Load(), 2)
	}
}
