...
func testSequentialAnnouncements(t *testing.T, protocol int) {
	...
	c.backend.fetcher.newHeadHook = func(header *types.Header) {
		importCh <- header
	}
	...
	verifyChainHeight(t, c.backend.fetcher, 4)
}
...
func testGappedAnnouncements(t *testing.T, protocol int) {
	...
	c.backend.fetcher.newHeadHook = func(header *types.Header) { done <- header }
	...
	<-done // Wait syncing
	verifyChainHeight(t, c.backend.fetcher, 5)
}
