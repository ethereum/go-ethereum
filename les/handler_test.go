...
func expectResponse(r p2p.MsgReader, mapping *lespay.MatchedHalfMapping, msgcode, reqID, bv, cost uint64, data interface{}) error {
	type resp struct {
		Meta replyMetaInfo
		Data interface{}
	}
	return p2p.ExpectMsg(r, msgcode, resp{replyMetaInfo{
		mapping:  mapping,
		reqID:    metaInfoField{value: reqID, set: true},
		bv:       metaInfoField{value: bv, set: true},
		realCost: metaInfoField{value: cost, set: true},
	}, data})
}
...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetBlockHeadersMsg, reqID, tt.query)
		if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, BlockHeadersMsg, reqID, testBufLimit, 0, headers); err != nil {
...
func testGetBlockBodies(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		// Send the hash request and verify the response
		sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetBlockBodiesMsg, reqID, hashes)
		if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, BlockBodiesMsg, reqID, testBufLimit, 0, bodies); err != nil {
...
func testGetCode(t *testing.T, protocol int) {
	...
	sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetCodeMsg, 42, codereqs)
	if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, CodeMsg, 42, testBufLimit, 0, codes); err != nil {
...
func testGetStaleCode(t *testing.T, protocol int) {
	...
	check := func(number uint64, expected [][]byte) {
		...
		sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetCodeMsg, 42, []*CodeReq{req})
		if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, CodeMsg, 42, testBufLimit, 0, expected); err != nil {
			t.Errorf("codes mismatch: %v", err)
		}
	}
...
func testGetReceipt(t *testing.T, protocol int) {
	...
	// Send the hash request and verify the response
	sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetReceiptsMsg, 42, hashes)
	if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, ReceiptsMsg, 42, testBufLimit, 0, receipts); err != nil {
...
func testGetProofs(t *testing.T, protocol int) {
	...
	sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetProofsV2Msg, 42, proofreqs)
	if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, ProofsV2Msg, 42, testBufLimit, 0, proofsV2.NodeList()); err != nil {
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		...
		sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetProofsV2Msg, 42, []*ProofReq{req})
		...
		if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, ProofsV2Msg, 42, testBufLimit, 0, expected); err != nil {
			t.Errorf("codes mismatch: %v", err)
		}
	}
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	// Send the proof request and verify the response
	sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetHelperTrieProofsMsg, 42, requestsV2)
	if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, HelperTrieProofsMsg, 42, testBufLimit, 0, proofsV2); err != nil {
		t.Errorf("proofs mismatch: %v", err)
	}
}
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		// Send the proof request and verify the response
		sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetHelperTrieProofsMsg, 42, requests)
		if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, HelperTrieProofsMsg, 42, testBufLimit, 0, proofs); err != nil {
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	test := func(tx *types.Transaction, send bool, expStatus light.TxStatus) {
		reqID++
		if send {
			sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, SendTxV2Msg, reqID, types.Transactions{tx})
		} else {
			sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetTxStatusMsg, reqID, []common.Hash{tx.Hash()})
		}
		if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, TxStatusMsg, reqID, testBufLimit, 0, []light.TxStatus{expStatus}); err != nil {
...
func testStopResume(t *testing.T, protocol int) {
	...
	req := func() {
		reqID++
		sendRequest(server.peer.app, server.peer.cpeer.mapping.Receive, GetBlockHeadersMsg, reqID, &getBlockHeadersData{Origin: hashOrNumber{Hash: header.Hash()}, Amount: 1})
	}
	for i := 1; i <= 5; i++ {
		...
		for expBuf >= testCost {
			req()
			expBuf -= testCost
			if err := expectResponse(server.peer.app, server.peer.cpeer.mapping.Send, BlockHeadersMsg, reqID, expBuf, testCost, []*types.Header{header}); err != nil {
