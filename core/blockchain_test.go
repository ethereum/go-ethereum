...
func testReorgBadHashes(t *testing.T, full bool, scheme string) {
	...
	ncm, err := NewBlockChain(blockchain.db, DefaultCacheConfigWithScheme(scheme), gspec, nil, ethash.NewFaker(), vm.Config{}, nil)
	if err != nil {
...
func testSideImportPrunedBlocks(t *testing.T, scheme string) {
	...
	// TODO (MariusVanDerWijden) I have no idea why this is supposed to not fail
	// Imo were reimporting stuff from the canonical chain that was pruned away,
	// so it should fail with pruned ancestor or something similar.
	// it fails with triedb parent layer missing, which sounds similar to me.
	// But I have no idea why it suddenly breaks after I removed the Forkchoicer
	/*
...
func TestEIP3651(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, engine, vm.Config{Tracer: logger.NewMarkdownLogger(&logger.Config{}, os.Stderr)}, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
