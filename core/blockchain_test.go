...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, engine, SetChainConfig(params.AllEthashProtocolChanges))
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(blockchain.db, ethash.NewFaker(), SetChainConfig(blockchain.chainConfig))
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, ethash.NewFaker(), SetChainConfig(gspec.Config))
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, ethash.NewFaker(), SetChainConfig(gspec.Config))
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, ethash.NewFaker(), SetChainConfig(gspec.Config))
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb,
		ethash.NewFaker(),
		SetCacheConfig(&archiveCaching),
		SetChainConfig(gspec.Config))
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, ethash.NewFaker(), SetChainConfig(gspec.Config))
...
func TestLogRebirth(t *testing.T) {
	var (
		...
		blockchain, _ = NewBlockChain(db, engine, SetChainConfig(gspec.Config))
	)
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb, engine, SetChainConfig(params.TestChainConfig))
	if err != nil {
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	gspec.MustCommit(ancientDb)
	ancientChain, _ := NewBlockChain(ancientDb, ethash.NewFaker(), SetChainConfig(gspec.Config))
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	chain, err := NewBlockChain(diskdb, runEngine, SetChainConfig(&chainConfig))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	chain, err := NewBlockChain(chaindb, engine, SetChainConfig(params.TestChainConfig))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	chain, err := NewBlockChain(chaindb, runEngine, SetChainConfig(&chainConfig))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	var (
...
func TestTransactionIndices(t *testing.T) {
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, ethash.NewFaker(), SetChainConfig(params.TestChainConfig), SetTxLookupLimit(&l))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func TestDeleteRecreateSlots(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb,
		engine,
...
func TestDeleteRecreateAccount(t *testing.T) {
	...
	diskdb := rawdb.NewMemoryDatabase()
	gspec.MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, engine,
		SetChainConfig(params.TestChainConfig),
		SetVmConfig(vmConfig))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
...
func TestEIP2718Transition(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb, engine, SetChainConfig(gspec.Config))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	if n, err := chain.InsertChain(blocks); err != nil {
