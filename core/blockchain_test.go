...
func TestEIP7002(t *testing.T) {
	var (
		...
		gspec  = &Genesis{
			...
			Alloc: types.GenesisAlloc{
				addr:                    {Balance: funds},
				params.ExitQueueAddress: {Code: common.FromHex("")},
			},
		}
	)
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, engine, 1, func(i int, b *BlockGen) {
		// One transaction to exit contract.
		txdata := &types.DynamicFeeTx{
			ChainID:    gspec.Config.ChainID,
			Nonce:      0,
	...
	exits := block.Exits()
	if len(exits) != 1 {
		t.Fatalf("wrong number of exits: wanted 1, got %d", len(exits))
	}
	if exits[0].Source != exit.Source {
		t.Fatalf("wrong source address: wanted %s, got %s", exit.Source, exits[0].Source)
	}
	if exits[0].PublicKey != exit.PublicKey {
		t.Fatalf("wrong public key: wanted %s, got %s", common.Bytes2Hex(exit.PublicKey[:]), common.Bytes2Hex(exits[0].PublicKey[:]))
	}
}
