...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(NewBlockChainConfig(db, nil, params.AllEthashProtocolChanges, engine, vm.Config{}, nil), nil)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(NewBlockChainConfig(blockchain.db, nil, blockchain.chainConfig, ethash.NewFaker(), vm.Config{}, nil), nil)
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(NewBlockChainConfig(archiveDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil), nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(NewBlockChainConfig(fastDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil), nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(NewBlockChainConfig(ancientDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil), nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewBlockChain(NewBlockChainConfig(archiveDb, &archiveCaching, gspec.Config, ethash.NewFaker(), vm.Config{}, nil), nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	defer delfn()
	light, _ := NewBlockChain(NewBlockChainConfig(lightDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil), nil)
...
func TestLogRebirth(t *testing.T) {
	var (
		...
		blockchain, _ = NewBlockChain(NewBlockChainConfig(db, nil, gspec.Config, engine, vm.Config{}, nil), nil)
	)
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	chain, err := NewBlockChain(NewBlockChainConfig(chaindb, nil, params.TestChainConfig, engine, vm.Config{}, nil), nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
