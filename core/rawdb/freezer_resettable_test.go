...
func TestResetFreezer(t *testing.T) {
	...
	f, _ := newResettableFreezer(t.TempDir(), "", false, freezerTestTableSize, freezerTestTableDef)
	defer f.Close()
...
func TestFreezerCleanup(t *testing.T) {
	...
	datadir := t.TempDir()
	f, _ := newResettableFreezer(datadir, "", false, freezerTestTableSize, freezerTestTableDef)
	f.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for _, item := range items {
			op.AppendRaw("test", item.id, item.blob)
		}
