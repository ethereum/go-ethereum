...
func ReadDatabaseVersion(db gdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db gdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db gdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db gdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
// given genesis hash.
func ReadGenesisStateSpec(db gdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(hash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db gdb.KeyValueWriter, hash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db gdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db gdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db gdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func ReadTransitionStatus(db gdb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the eth2 transition status to the database
func WriteTransitionStatus(db gdb.KeyValueWriter, data []byte) {
