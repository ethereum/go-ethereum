...
type freezerTableMeta struct {
	...
	// volatile and should be recoverable after an unclean shutdown. If data
	// integrity is required, manually force a sync of the freezer before proceeding
	// with further operations (e.g. freezer.Sync() first and then write data to
	// key value store in some circumstances).
	//
...
func decodeV1(file *os.File) *freezerTableMeta {
	...
	return &freezerTableMeta{
		file:        file,
		version:     freezerTableV2, // silent upgrade
		virtualTail: o.Tail,
	}
}
...
// or by constructing a new one from scratch. If legacy metadata is detected,
// it will be upgraded to the latest version, with an additional flag returned.
func newMetadata(file *os.File) (*freezerTableMeta, bool, error) {
	stat, err := file.Stat()
	if err != nil {
	...
	if stat.Size() == 0 {
		...
		return m, false, nil // metadata is not found, new freezer table
	}
	if m := decodeV2(file); m != nil {
		return m, false, nil
	}
	if m := decodeV1(file); m != nil {
		return m, true, nil // legacy metadata
	}
	return nil, false, errors.New("failed to decode metadata")
}
