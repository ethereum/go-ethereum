...
type freezerTableMeta struct {
	...
	// integrity is required, manually force a sync of the freezer before
	// proceeding with further operations.
	//
	// The offset could be moved forward by applying sync operation, or be moved
	// backward in cases of head/tail truncation.
	flushOffset uint64
}
...
func newMetadata(file *os.File) (*freezerTableMeta, bool, error) {
	...
	if m := decodeV1(file); m != nil {
		// Upgrade the existent legacy metadata to new version
		if err := m.write(true); err != nil {
			return nil, false, err
		}
