...
type freezerTable struct {
	...
	legacy   bool              // flag whether the legacy data is detected
...
func newTable(path string, name string, readMeter metrics.Meter, writeMeter metrics.Meter, sizeGauge metrics.Gauge, maxFilesize uint32, noCompression, readonly bool) (*freezerTable, error) {
	...
	// is detected.
	metadata, legacy, err := newMetadata(meta)
	if err != nil {
		return nil, err
	}
...
func (t *freezerTable) repairIndex() error {
	...
	if t.legacy {
		t.legacy = false
		t.logger.Info("Recover the flush offset", "offset", offset)
	...
	if offset > int64(t.metadata.flushOffset) {
		...
		t.logger.Info("Truncate excessive items above the flush offset", "size", size)
	...
	// possible scenario for it is power failure or system crash occurs after
	// truncating the tail segment in index file without updating the flush offset.
	// In this case, automatically reset the flush offset with the file size which
	// implicitly means the entire index file is complete.
	if t.readonly {
...
// extended with invalid "garbage" data (normally zero bytes) and then afterwards
...
func (t *freezerTable) truncateTail(items uint64) error {
	...
	// Sync the table before perform the index tail truncation. A crash may occur
	// after truncating the index file without updating the flush offset, leaving
	// a dangling offset that points to a position outside the file. The offset
	// will be rewound to the end of file during the next run automatically and
	// implicitly assumes all the items within the file are complete. Therefore,
	// forcibly flush everything above the offset to satisfy the assumption.
	...
	// tail truncation and all the items in these files are regarded as correct.
	shorten := indexEntrySize * (newDeleted - deleted)
	if t.metadata.flushOffset <= shorten {
		// It's never expected to happen
		return fmt.Errorf("invalid index flush offset: %d, shorten: %d", t.tailId, shorten)
	} else {
