...
func (f *Freezer) ReadAncients(fn func(gdb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(gdb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
