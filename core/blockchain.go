...
func (bc *BlockChain) Export(w io.Writer, receipts bool) error {
	return bc.ExportN(w, uint64(0), bc.CurrentBlock().NumberU64(), receipts)
}

// ExportN writes a subset of the active chain to the given writer.
func (bc *BlockChain) ExportN(w io.Writer, first uint64, last uint64, receipts bool) error {
	if !bc.chainmu.TryLock() {
		return errChainStopped
	}
	defer bc.chainmu.Unlock()
