...
func (iv *IndexView) GetFilterMapRows(mapIndices []uint32, rowIndex, layers uint32) (rows []FilterRow, err error) {
	...
	for i := dbIndices; i < len(mapIndices); i++ {
		...
		if mapIndex < iv.headMapIndex {
			row = iv.finishedMaps[mapIndex-iv.firstMemoryMap].getRow(rowIndex, iv.storage.params.getMaxRowLength(layers))
		}
...
func (rs *renderState) addReceipts(receipts types.Receipts) {
	...
	//fmt.Println("addReceipts", rs.mapIndex, rs.lvPointer%rs.params.valuesPerMap)
	...
	//fmt.Println(" done      ", rs.mapIndex, rs.lvPointer%rs.params.valuesPerMap)
}
