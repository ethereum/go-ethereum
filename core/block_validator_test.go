...
func TestHeaderVerification(t *testing.T) {
	...
	chain, _ := NewBlockChain(testdb, ethash.NewFaker(), SetChainConfig(params.TestChainConfig))
	defer chain.Stop()
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	// Run the header checker for blocks one-by-one, checking for both valid and invalid nonces
	chain, _ := NewBlockChain(testdb, runEngine, SetChainConfig(chainConfig))
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, ethash.NewFakeFailer(uint64(len(headers)-1)), SetChainConfig(params.TestChainConfig))
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	chain, _ := NewBlockChain(testdb, ethash.NewFakeDelayer(time.Millisecond), SetChainConfig(params.TestChainConfig))
	defer chain.Stop()
