...
func TestIndexServer(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, chain[0], ethash.NewFaker(), db, 200, func(i int, gen *BlockGen) {})
	chain = append(chain, blocks...)

	run(func() {
		blockchain.RegisterIndexer(ti, "")
		testSuspendHookCh = blockchain.indexServers.servers[0].testSuspendHookCh
	})
