...
func overrideStates(states map[common.Hash]common.Hash) *map[common.Hash]common.Hash {
	return &states
}
...
func TestStateOverride(t *testing.T) {
	var cases = []struct {
		...
		// override storage by diff
		{
			map[common.Address]OverrideAccount{
				testAllocAddr: {
					StateDiff: overrideStates(map[common.Hash]common.Hash{
		...
		// override storage by replacing entire storage
		{
			map[common.Address]OverrideAccount{
				testAllocAddr: {
					State: overrideStates(map[common.Hash]common.Hash{
						testAllocSlotKey2: {0x2},
					}),
				},
			},
