...
// with their values be tracked as original value.
func (s *StateDB) handleDestruction(rawStorageKey bool) (map[common.Hash]*accountDelete, []*trienode.NodeSet, error) {
	var (
...
func (s *StateDB) commit(deleteEmptyObjects bool, rawStorageKey bool) (*stateUpdate, error) {
	...
	for addr, op := range s.mutations {
		...
		workers.Go(func() error {
			...
			update, set, err := obj.commit(rawStorageKey)
			if err != nil {
				return err
			}
			if err := merge(set); err != nil {
	...
	return newStateUpdate(rawStorageKey, origin, root, deletes, updates, nodes), nil
}

// commitAndFlush is a wrapper of commit which also commits the state mutations
// to the configured data stores.
func (s *StateDB) commitAndFlush(block uint64, deleteEmptyObjects bool, rawStorageKey bool) (*stateUpdate, error) {
	ret, err := s.commit(deleteEmptyObjects, rawStorageKey)
	if err != nil {
		return nil, err
	}
...
//
// rawStorageKey is a flag indicating whether storage wiping is permitted.
// Since self-destruction was deprecated with the Cancun fork and there are
// no empty accounts left that could be deleted by EIP-158, storage wiping
// should not occur.
func (s *StateDB) Commit(block uint64, deleteEmptyObjects bool, rawStorageKey bool) (common.Hash, error) {
	ret, err := s.commitAndFlush(block, deleteEmptyObjects, rawStorageKey)
	if err != nil {
