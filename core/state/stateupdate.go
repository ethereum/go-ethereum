...
type accountUpdate struct {
	...
	// storagesOriginByKey stores the original values of mutated slots
	// in prefix-zero-trimmed RLP format. The map key refers to the raw
	// storage slot key.
	storagesOriginByKey map[common.Hash][]byte

	// storagesOriginByHash stores the original values of mutated slots
	// in prefix-zero-trimmed RLP format. The map key refers to the hash
	// of the storage slot key.
...
type stateUpdate struct {
	...
	// (a) the value is keyed by account hash and **storage slot key** if rawSlotKey is true;
	// (b) the value is keyed by account hash and **storage slot key hash** if rawSlotKey is false;
...
func newStateUpdate(rawSlotKey bool, originRoot common.Hash, root common.Hash, deletes map[common.Hash]*accountDelete, updates map[common.Hash]*accountUpdate, nodes *trienode.MergedNodeSet) *stateUpdate {
	var (
	...
	for addrHash, op := range updates {
		...
		origins := op.storagesOriginByHash
		if rawSlotKey {
			origins = op.storagesOriginByKey
		}
		if len(origins) > 0 {
			origin := storagesOrigin[addr]
			if origin == nil {
				storagesOrigin[addr] = origins
				continue
			}
			for key, slot := range origins {
