...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(uint256.Int).SetUint64(params.Ether), live.BalanceChangeUnspecified)
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(uint256.Int).SetUint64(params.Ether), live.BalanceChangeUnspecified)
...
func testAddBalance(pool *LegacyPool, addr common.Address, amount *big.Int) {
	pool.mu.Lock()
	pool.currentState.AddBalance(addr, uint256.MustFromBig(amount), live.BalanceChangeUnspecified)
	pool.mu.Unlock()
}
...
func TestChainFork(t *testing.T) {
	...
	resetState := func() {
		statedb, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
		statedb.AddBalance(addr, uint256.NewInt(100000000000000), live.BalanceChangeUnspecified)
...
func BenchmarkMultiAccountBatchInsert(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		key, _ := crypto.GenerateKey()
		account := crypto.PubkeyToAddress(key.PublicKey)
		pool.currentState.AddBalance(account, uint256.NewInt(1000000), live.BalanceChangeUnspecified)
