...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	block.LCPContext = getDposContextTrie(db.(ethdb.Database), header)
	return block
}

func getDposContextTrie(db ethdb.Database, header *types.Header) *types.LCPContext {
	dposContestProto := header.LCPContext
	if dposContestProto != nil {
		dposContext, err := types.NewLCPContextFromProto(db, dposContestProto)
		if err != nil {
			return nil
		}
		return dposContext
