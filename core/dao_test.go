...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, ethash.NewFaker(), SetChainConfig(&proConf))
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, ethash.NewFaker(), SetChainConfig(&conConf))
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db = rawdb.NewMemoryDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, ethash.NewFaker(), SetChainConfig(&conConf))
		defer bc.Stop()
		...
		db = rawdb.NewMemoryDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, ethash.NewFaker(), SetChainConfig(&proConf))
		defer bc.Stop()
