...
func (ga *GenesisAlloc) deriveHash(cfg *params.ChainConfig) (common.Hash, error) {
	...
	db := state.NewDatabaseWithConfig(rawdb.NewMemoryDatabase(), &trie.Config{UseVerkle: (cfg != nil) && cfg.IsCancun(big.NewInt(int64(0)))})
...
func (g *Genesis) Commit(db ethdb.Database) (*types.Block, error) {
	...
	// database.
	if err := g.Alloc.flush(db, config); err != nil {
		return nil, err
	}
