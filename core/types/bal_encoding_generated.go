...
// Hash: 0517f4fea59f33d3fe2c7fc0aa50c36276f2c8090f07b99bafbd028172244a65
// Version: 0.1.3
package types
...
func (e *encodingAccountBalanceDiff) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'Changes'
	if size := len(e.Changes); size > 300000 {
		err = ssz.ErrListTooBigFn("encodingAccountBalanceDiff.Changes", size, 300000)
		return
	}
...
// MarshalSSZ ssz marshals the encodingAccountNonces object
func (e *encodingAccountNonces) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(e)
}

// MarshalSSZTo ssz marshals the encodingAccountNonces object to a target array
func (e *encodingAccountNonces) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(24)
	...
	// Field (1) 'Diffs'
	if size := len(e.Diffs); size > 300000 {
		err = ssz.ErrListTooBigFn("encodingAccountNonces.Diffs", size, 300000)
		return
	}
	for ii := 0; ii < len(e.Diffs); ii++ {
		if dst, err = e.Diffs[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
// UnmarshalSSZ ssz unmarshals the encodingAccountNonces object
func (e *encodingAccountNonces) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 24 {
		return ssz.ErrSize
	...
	// Field (1) 'Diffs'
	{
		...
		e.Diffs = make([]encodingAccountNonce, num)
		for ii := 0; ii < num; ii++ {
			if err = e.Diffs[ii].UnmarshalSSZ(buf[ii*16 : (ii+1)*16]); err != nil {
				return err
			}
		}
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the encodingAccountNonces object
func (e *encodingAccountNonces) SizeSSZ() (size int) {
	size = 24
...
// HashTreeRoot ssz hashes the encodingAccountNonces object
func (e *encodingAccountNonces) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(e)
}

// HashTreeRootWith ssz hashes the encodingAccountNonces object with a hasher
func (e *encodingAccountNonces) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()
	...
	{
		subIndx := hh.Index()
		num := uint64(len(e.Diffs))
		if num > 300000 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range e.Diffs {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
...
// GetTree ssz hashes the encodingAccountNonces object
func (e *encodingAccountNonces) GetTree() (*ssz.Node, error) {
	return ssz.ProofTree(e)
}
...
func (e *encodingBlockAccessList) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(e.AccountAccesses); size > 300000 {
		err = ssz.ErrListTooBigFn("encodingBlockAccessList.AccountAccesses", size, 300000)
		return
	}
	{
	...
	if size := len(e.BalanceDiffs); size > 300000 {
		err = ssz.ErrListTooBigFn("encodingBlockAccessList.BalanceDiffs", size, 300000)
		return
	}
	{
	...
	// Field (2) 'CodeDiffs'
	if size := len(e.CodeDiffs); size > 300000 {
		err = ssz.ErrListTooBigFn("encodingBlockAccessList.CodeDiffs", size, 300000)
		return
	}
	{
	...
	// Field (3) 'NonceDiffs'
	if size := len(e.NonceDiffs); size > 30000 {
		err = ssz.ErrListTooBigFn("encodingBlockAccessList.NonceDiffs", size, 30000)
		return
	}
	{
		offset = 4 * len(e.NonceDiffs)
		for ii := 0; ii < len(e.NonceDiffs); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += e.NonceDiffs[ii].SizeSSZ()
		}
	}
	for ii := 0; ii < len(e.NonceDiffs); ii++ {
...
func (e *encodingBlockAccessList) UnmarshalSSZ(buf []byte) error {
	...
	// Field (3) 'NonceDiffs'
	{
		...
		e.NonceDiffs = make([]encodingAccountNonces, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if err = e.NonceDiffs[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
