...
type txJSON struct {
	...
	InitCodes            *[][]byte       `json:"initCode,omitempty"`
...
func (tx *Transaction) UnmarshalJSON(input []byte) error {
	...
	case InitCodeTxType:
		...
		if dec.InitCodes == nil {
			return errors.New("missing required field 'initcodes' in transaction")
		}
		if len(*dec.InitCodes) > params.MaxInitCodeCount {
			return errors.New("too many initcodes in initcode transaction")
		}
		for _, code := range *dec.InitCodes {
			if len(code) == 0 {
				return errors.New("initcode too short in initcode transaction")
			}
			if len(code) > params.MaxInitCodeSize {
				return errors.New("initcode too long in initcode transaction")
			}
		}
