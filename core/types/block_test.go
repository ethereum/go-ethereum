...
func TestACLJsonFiles(t *testing.T) {
	finfo, err := ioutil.ReadDir("./testdata")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, f := range finfo {
		...
		data, err := ioutil.ReadFile(fmt.Sprintf("./testdata/%v", f.Name()))
		if err != nil {
			t.Fatalf("testfile %v: error %v", f.Name(), err)
		}
		if err := json.Unmarshal(data, &tc); err != nil {
			t.Fatalf("testfile %v: error %v", f.Name(), err)
		}
		var block Block
		if err := rlp.DecodeBytes(common.FromHex(tc.Rlp), &block); err != nil {
			t.Fatalf("testfile %v, decode error: %v", f.Name(), err)
		}
