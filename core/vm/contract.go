...
func (c *Contract) UseGas(gas uint64, tracer tracer, reason tracing.GasChangeReason) (ok bool) {
	if c.Gas < gas {
		return false
	}
...
func (c *Contract) RefundGas(gas uint64, tracer tracer, reason tracing.GasChangeReason) {
	if gas == 0 {
		return
	}
	if reason != tracing.GasChangeIgnored {
