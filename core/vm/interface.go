...
type StateDB interface {
	...
	// For non-existent addresses or self-destructed addresses (during the tx execution)
	// it returns zero
	GetBalance(common.Address) *big.Int
	// GetNonce returns the nonce for the given address
	// For non-existent addresses it returns zero
	GetNonce(common.Address) uint64
	// SetNonce sets the nonce for the given address
	SetNonce(common.Address, uint64)
	// GetCodeHash returns the codeHash for the given address
	// For non-existent addresses or addresses without a code (EOAs) it returns types.EmptyCodeHash
	GetCodeHash(common.Address) common.Hash
	// GetCode returns the code for the given address,
	// For non-existent addresses or addresses without a code (EOAs) it returns nil
	GetCode(common.Address) []byte
	// SetCode sets the code for the given address
	SetCode(common.Address, []byte)
	// GetCodeSize returns the size of the code for the given address
	// For non-existent addresses or addresses without a code (EOAs) it returns zero
	...
	// For non-existent addresses or non-existent keys it returns empty hash (common.Hash{})
	...
	// If a value already exist at the given pair, it will be replaced.
	...
	// Check EIP-1153 for more info about transient state
	...
	// Notably this returns true for self-destructed accounts.
	Exist(common.Address) bool
	// Empty returns whether the given account is empty. Empty
	// is defined according to EIP-161 (balance = nonce = code = 0).
	Empty(common.Address) bool
	// AddressInAccessList checks if an address is in the access list (see EIP-2930).
	AddressInAccessList(addr common.Address) bool
	// SlotInAccessList checks if an slot is in the access list (see EIP-2930).
	SlotInAccessList(addr common.Address, slot common.Hash) (addressOk bool, slotOk bool)
	// AddAddressToAccessList adds the given address to the access list. This operation is safe to perform
	// even if the feature/fork is not active yet (see EIP-2930).
	...
	// Snapshot takes a snapshot and returns an integer that can be used later for RevertToSnapshot
	// Nested snapshoting is be supported.
	Snapshot() int
