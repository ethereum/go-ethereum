...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, evm.interpreter, &ScopeContext{Stack: stack})
		if len(stack.data) != 1 {
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		stack.push(y)
		stack.push(x)
		opAddmod(&pc, evm.interpreter, &ScopeContext{Stack: stack})
		actual := stack.pop()
		if actual.Cmp(expected) != 0 {
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evm.interpreter, &ScopeContext{Memory: mem, Stack: stack})
...
func BenchmarkOpKeccak256(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		stack.push(start)
		opKeccak256(&pc, evm.interpreter, &ScopeContext{Memory: mem, Stack: stack})
	}
}
...
func TestRandom(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		opRandom(&pc, evm.interpreter, &ScopeContext{Stack: stack})
		if len(stack.data) != 1 {
...
func TestBlobHash(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		opBlobHash(&pc, evm.interpreter, &ScopeContext{Stack: stack})
		if len(stack.data) != 1 {
			t.Errorf("Expected one item on stack after %v, got %d: ", tt.name, len(stack.data))
		}
		actual := stack.pop()
...
func TestOpMCopy(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		// Do the copy
		opMcopy(&pc, evm.interpreter, &ScopeContext{Memory: mem, Stack: stack})
