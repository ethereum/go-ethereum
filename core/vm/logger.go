...
type AccessListTracer struct {
	old  *types.AccessList
	list accessList
	err  error
}

func NewAccessListTracer(list *types.AccessList) *AccessListTracer {
...
func (a *AccessListTracer) CaptureStart(env *EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int)  {
	env.StateDB.PrepareAccessList(from, &to, env.ActivePrecompiles(), *a.old)
}

func (a *AccessListTracer) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, rData []byte, contract *Contract, depth int, err error)  {´
	...
	if (op == DELEGATECALL || op == CALL || op == STATICCALL || op == CALLCODE) && stack.len() >= 5 {
		stackElem := stack.data[stack.len()-5].Bytes32()
		address := common.BytesToAddress(stackElem[:])
		a.list.AddAddress(address)
	}
}

func (*AccessListTracer) CaptureFault(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) {
	// ignore faults
}
