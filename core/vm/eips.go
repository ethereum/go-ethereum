...
func opExtCodeCopyEIP4762(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	scope.Contract.UseGas(consumed, interpreter.evm.tracer, tracing.GasChangeUnspecified)
	if consumed < wanted {
	...
	*pc += 1
	if *pc < codeLen {
		...
		if !scope.Contract.IsDeployment && !scope.Contract.IsSystemCall && *pc%31 == 0 {
			...
			scope.Contract.UseGas(wanted, interpreter.evm.tracer, tracing.GasChangeUnspecified)
			if consumed < wanted {
				return nil, ErrOutOfGas
			}
		}
