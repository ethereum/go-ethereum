...
func NewChainManager(chainDb ethdb.Database, pow pow.PoW, mux *event.TypeMux) (*ChainManager, error) {
	...
	for hash, _ := range BadHashes {
		if block := bc.GetBlock(hash); block != nil {
			glog.V(logger.Error).Infof("Found bad hash, rewinding chain to block #d [%x…]", block.NumberU64(), block.ParentHash().Bytes()[:4])
...
// header to this very same block to prevent the headers from divergind on a
...
func (self *ChainManager) InsertHeaderChain(chain []*types.Header, verify bool) (int, error) {
	...
	for i, header := range chain {
		...
		if BadHashes[hash] {
			...
			return i, fmt.Errorf("Bad header %d [%x…], known bad hash", header.Number, hash)
		}
