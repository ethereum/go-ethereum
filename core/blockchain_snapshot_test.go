...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(NewBlockChainConfig(db, cacheConfig, params.AllEthashProtocolChanges, engine, vm.Config{}, nil), nil)
	if err != nil {
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(NewBlockChainConfig(snaptest.db, nil, params.AllEthashProtocolChanges, snaptest.engine, vm.Config{}, nil), nil)
	if err != nil {
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(NewBlockChainConfig(newdb, nil, params.AllEthashProtocolChanges, snaptest.engine, vm.Config{}, nil), nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(NewBlockChainConfig(snaptest.db, cacheConfig, params.AllEthashProtocolChanges, snaptest.engine, vm.Config{}, nil), nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(NewBlockChainConfig(snaptest.db, config, params.AllEthashProtocolChanges, snaptest.engine, vm.Config{}, nil), nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newBlocks, _ := GenerateChain(params.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.newBlocks, func(i int, b *BlockGen) {})
