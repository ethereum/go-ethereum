...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal, exits []*types.Exit) (*types.Block, error) {
	if !beacon.IsPoSHeader(header) {
		return beacon.ethone.FinalizeAndAssemble(chain, header, state, txs, uncles, receipts, nil, nil)
	}
	shanghai := chain.Config().IsShanghai(header.Number, header.Time)
	if shanghai {
	...
	return types.NewBlockWithExits(header, txs, uncles, receipts, withdrawals, exits, trie.NewStackTrie(nil)), nil
}
